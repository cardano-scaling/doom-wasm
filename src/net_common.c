...
void NET_Conn_Run(net_connection_t *conn)
{
    ...
    if (conn->state == NET_CONN_STATE_CONNECTED) {
        ...
        if (conn->reliable_packets != NULL &&
            (conn->reliable_packets->last_send_time < 0 || nowtime - conn->reliable_packets->last_send_time > 1000)) {
...
net_packet_t *NET_Conn_NewReliable(net_connection_t *conn, int packet_type)
{
    ...
    for (listend = &conn->reliable_packets; *listend != NULL; listend = &((*listend)->next))
        ;
...
{
    if (settings->ticdup <= 0) return false;

    if (settings->extratics < 0) return false;

    if (settings->deathmatch < 0 || settings->deathmatch > 2) return false;

    if (settings->skill < sk_noitems || settings->skill > sk_nightmare) return false;

    if (!D_ValidGameVersion(mission, settings->gameversion)) return false;

    if (!D_ValidEpisodeMap(mission, mode, settings->episode, settings->map)) return false;
...
void NET_LogPacket(net_packet_t *packet)
{
    ...
    printf("NET: \t%02x", packet->data[packet->pos]);
    for (i = 1; i < bytes; ++i) {
        if ((i % 16) == 0) {
            printf("\nNET: \t");
        }
        ...
        printf("%02x", packet->data[packet->pos + i]);
    }
    printf("\n");
}
